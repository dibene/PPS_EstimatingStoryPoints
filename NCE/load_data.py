...
def prepare_lm(seqs, vocab_size=10000, max_len=100):
    ...
    # como mucho max length palabras
    # lengths array longitud de cada sentencia como min 100
    ...
    mask = numpy.zeros((n_samples, maxlen)).astype('int64')
    # x y mask iguales inicializados con 0 de n_samplesXmaxlen
    idx = 0
    # recorre sentencias y aplana en x e y
    for i, s in enumerate(seqs):
        l = lengths[i]
        if l < 1: continue
        mask[idx, :l] = 1
